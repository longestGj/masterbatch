...
These are action names. The single Page Spec stores status as `PLANNED / READY / BUILDING / REVIEW / ACCEPTED / RELEASED`; ACCEPTED means the applicable review passed, not permission to publish.
...
**RELEASE** is a separate authorized operation after applicable checks pass. The reviewed Page Spec can be ACCEPTED while awaiting explicit publication approval and the conditions in [RELEASE](RELEASE.md). After deploy and smoke checks, set RELEASED.

Current page status lives **only in its Page Spec**. SITE_MAP and SEO_MAP are lookup/ownership tables, Git preserves document history, WordPress revisions preserve supported editorial history, and backups preserve runtime data. None replaces the others. A normal task needs a Page Spec, a scoped Git change in this repository, and a review result, not extra lifecycle manifests.
